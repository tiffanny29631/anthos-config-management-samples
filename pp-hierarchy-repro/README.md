...
- **KNV2009 / `cannot change roleRef` on RoleBinding** — Kubernetes does not allow updating `roleRef` on an existing RoleBinding. This sample uses a dedicated name (`pp-hierarchy-repro-repo-sync`) and `ClusterRole/edit` (same pattern as other ACM RepoSync examples). If you still manage a conflicting object named `repo-sync` in `team-a`, delete it from the cluster or from Git so Config Sync can reconcile cleanly.
